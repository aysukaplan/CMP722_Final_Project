...
def setup_logging(log_dir="log"):
    ...
    log_filename = os.path.join(log_dir, f"baseline_without_dif_gaussian_512_{current_time}.log")
...
# changed sigma_scale to 1/5 from 1/8
def get_gaussian_kernel(patch_size, sigma_scale=1/5):
    h, w = patch_size
...
def main():
    ...
    logger.info(f"Loading SegFormer model from: {seg_model_id}")
    
    segmenter = SegformerForSemanticSegmentation.from_pretrained(seg_model_id)
    ...
    for dataset_name, loader in val_loaders.items():
        ...
        for i, (image_patches, mask_patches, coords, img_names) in enumerate(pbar):
            ...
            if current_processing_img_idx != -1 and patch_img_idx != current_processing_img_idx:
                ...
                if len(per_image_miou_list) < 3:
                     ...
                     save_segmentation_vis(
                        ...
                        output_dir="vis_results/base_without_dif_gaussian_512/"
                     )
                # ---------------------
            ...
            with torch.no_grad():
                ...
                patch_logits = F.interpolate(outputs.logits, size=SD_PATCH_SIZE, mode="bilinear", align_corners=False).cpu()
        ...
        if current_processing_img_idx != -1 and current_processing_img_idx in all_stitched_preds:
            ...
            if len(per_image_miou_list) < 3:
                ...
                save_segmentation_vis(
                    ...
                    full_img_path,
                    f"{dataset_name}_{last_processed_img_name}", 
                    "vis_results/base_without_dif_gaussian_512/"
                )
