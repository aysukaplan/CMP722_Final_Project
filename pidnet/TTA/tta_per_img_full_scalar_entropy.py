...
def setup_logging(log_dir="log"):
    ...
    log_filename = os.path.join(log_dir, f"tta_steering_full_1024_512_w50_ss45_{current_time}.log")
...
def preprocess_for_pidnet(image_tensor, device):
...
def adapt_image_with_steering(
    image, vae, unet, scheduler, segmenter, uncond_embeds, args, device, generator
):
    ...
    for t in tqdm(timesteps, desc="Steering", leave=False):
        ...
        # Enable Gradients for Steering on the Latent
        latent = latent.detach().requires_grad_(True)
        
        timestep_batch = torch.full((batch_size,), t.item(), device=device, dtype=torch.long)
        
        # --- MODIFICATION START ---
        # A. Predict Noise (Gradient ENABLED)
        # We allow PyTorch to build the graph through the U-Net here.
        noise_pred = unet(
            latent, timestep_batch, encoder_hidden_states=uncond_embeds
        ).sample
        
        # B. Estimate x0 (Differentiable w.r.t Latent AND U-Net)
        alpha_prod_t = scheduler.alphas_cumprod[t]
        beta_prod_t = 1 - alpha_prod_t
        
        # The gradient now flows via two paths:
        # 1. Directly from 'latent'
        # 2. Through 'noise_pred' (back into the U-Net)
        x0_pred_latent = (latent - beta_prod_t ** 0.5 * noise_pred) / alpha_prod_t ** 0.5
        # --- MODIFICATION END ---
        
        # C. Decode to Image (Differentiable through VAE)
        # Since you have enough VRAM, we decode the full batch with gradients.
        ...
        x0_pred_image_processed = preprocess_for_pidnet(x0_pred_image_01, device)
        logits = segmenter(x0_pred_image_processed)
        

        entropy = calculate_mean_entropy(logits.float())
        
        # 3. Calculate Gradient 
        # The autograd engine will now backpropagate through:
        # Segmenter -> VAE -> x0 calculation -> U-Net -> Latent
        grad = torch.autograd.grad(entropy, latent)[0]
        
        # Cleanup intermediate graph nodes to free memory for the next iteration,
        # but KEEP noise_pred for the scheduler step.
        ...
        with torch.no_grad():
            ...
            # We reuse the noise_pred we calculated earlier (detached from the graph).
            # This is faster than running unet(latent_steered).
            latent = scheduler.step(noise_pred.detach(), t, latent_steered).prev_sample
...
def create_argparser():
    defaults = dict(
        ...
        start_timestep=45, # <--- Corresponds to 'start_step_idx' in previous code
        guidance_scale=50, # Entropy scale
    ...
    for dataset_name, loader in val_loaders.items():
        ...
        for i, (image_patches, mask_patches, coords, img_names) in enumerate(pbar):
            ...
            generated_patches = adapt_image_with_steering(
                image_patches, vae, unet, scheduler, segmenter, 
                 batch_emb, args, device, batch_generator
            )
            ...
            with torch.no_grad():
                ...
                batch_logits = F.interpolate(outputs, size=SD_PATCH_SIZE, mode="bilinear", align_corners=False).cpu()
                ...
                if current_processing_img_idx != -1 and patch_img_idx != current_processing_img_idx:
                    ...
                    if len(per_image_miou_list) < 3:
                        ...
                        save_segmentation_vis(
                            ...
                            output_dir="vis_results/tta_steering_full_1024_512_w50_ss45/"
                        )
        ...
        if current_processing_img_idx != -1 and current_processing_img_idx in all_stitched_preds:
            ...
            if len(per_image_miou_list) < 3:
                seq_name = last_processed_img_name.split('_')[0]
                full_img_path = os.path.join(args.r, dataset_name, "val", seq_name, last_processed_img_name)
                save_segmentation_vis(final_pred, final_label, final_diff_img, full_img_path, f"{dataset_name}_{last_processed_img_name}", "vis_results/tta_steering_full_1024_512_w50_ss45/")
